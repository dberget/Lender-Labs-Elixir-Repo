...
function computeBaseFactorFromFeeBps(binStep, feeBps) {
    ...
    return new BN3(computedBaseFactor);
}
function getQPriceFromId(binId, binStep) {
    const bps = binStep.shln(SCALE_OFFSET).div(new BN3(BASIS_POINT_MAX));
...
function findSwappableMinMaxBinId(binStep) {
    ...
    let minBinId = new BN3(n.neg().toString());
    let maxBinId = new BN3(n.toString());
    let minQPrice = new BN3(1);
    let maxQPrice = new BN3("340282366920938463463374607431768211455");
    while (true) {
        const qPrice = getQPriceFromId(minBinId, binStep);
        if (qPrice.gt(minQPrice)) {
            break;
        } else {
            minBinId = minBinId.add(new BN3(1));
        }
    }
    while (true) {
        ...
        if (qPrice.lt(maxQPrice)) {
            ...
            maxBinId = maxBinId.sub(new BN3(1));
        }
    }
    return {
        minBinId,
...
function extensionBitmapRange() {
    return [
        BIN_ARRAY_BITMAP_SIZE.neg().mul(
            EXTENSION_BINARRAY_BITMAP_SIZE.add(new BN4(1))
        ),
...
function toAmountBidSide(activeId, totalAmount, distributions) {
    ...
    return distributions.map((bin) => {
        if (bin.binId > activeId) {
            ...
            return {
                binId: bin.binId,
                amount: new BN5(new Decimal2(totalAmount.toString()).mul(new Decimal2(bin.weight).div(totalWeight)).floor().toString())
            };
        }
    });
}
function toAmountAskSide(activeId, binStep, totalAmount, distributions) {
    ...
    return distributions.map((bin) => {
        if (bin.binId < activeId) {
            ...
            return {
                binId: bin.binId,
                amount: new BN5(new Decimal2(totalAmount.toString()).mul(weightPerPrice).div(totalWeight).floor().toString())
            };
        }
    });
}
...
var DLMM = class {
    ...
    static async createLbPair(connection, funder, tokenX, tokenY, presetParameter, activeId, opt) {
        ...
        const binStep = new BN9(presetParameterState.binStep);
     ...
     * @returns {Promise<Transaction>}
     */
    async updateWhitelistedWallet(walletToWhitelist, index) {
        const emptyIndex = this.lbPair.whitelistedWallet.findIndex(
            (pk) => pk.equals(PublicKey5.default)
        );
        ...
        const updateWhitelistedWalletTx = await this.program.methods.updateWhitelistedWallet(idx, walletToWhitelist).accounts({
            lbPair: this.pubkey,
